...
const EN = {
  ...
  'settings.sidebarSelection.name': 'Show selected text in sidebar',
  'settings.sidebarSelection.desc': 'Automatically mirror the current editor selection into the converter sidebar. Turn this off to keep the sidebar contents unchanged while selecting text; explicit sidebar commands still send content there.',
...
const ZH_CN: Record<TranslationKey, string> = {
  ...
  'settings.sidebarSelection.name': '在边栏显示所选内容',
  'settings.sidebarSelection.desc': '自动将当前编辑器选中的文本同步到右侧转换栏。关闭后，选择文本时不会覆盖边栏现有内容；通过命令或右键显式打开边栏时仍会传入对应内容。',
